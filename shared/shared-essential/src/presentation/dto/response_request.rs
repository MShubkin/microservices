...
///
/// `Ord` намеренен: порядок объявления вариантов определяет "тяжесть" —
/// `Success < Information < Warning < Error < Stop`. Это позволяет `Messages`
/// автоматически обновлять общий `kind` через `if message.kind > self.kind`,
/// не перечисляя явно все комбинации.
///
/// `None` — для совместимости с monolith, который возвращает это значение
/// в некоторых ответах. В новом коде не использовать.
#[derive(
    Debug,
    Default,
    Serialize,
