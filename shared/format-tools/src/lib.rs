//! Процедурные макросы для форматирования строк.
//!
//! `fomat!` — альтернатива `format!` с настраиваемыми разделителями.
//! Полезно там, где в шаблоне встречаются `{` и `}` как литеральные символы
//! (например, в шаблонах JSON или SQL), чтобы не экранировать их попарно.
//!
//! `numeric_format!` — форматирование числительных с учётом формы слова:
//! "1 маршрут", "2 маршрута", "5 маршрутов". Реализует склонение по правилам
//! русского языка через `numeral_relation` из `shared-essential`.
mod fomat;
mod numeric_format;
