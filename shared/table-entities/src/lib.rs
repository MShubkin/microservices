//! Сущности базы данных и доменные типы АСЭЗ.
//!
//! Крейт содержит SQLx-маппинги таблиц. Большинство сущностей — тонкие обёртки
//! над строками БД, без бизнес-логики: логика живёт в сервисах.
//!
//! `#![allow(ambiguous_glob_reexports)]` нужен потому, что несколько субмодулей
//! экспортируют типы с одинаковыми именами (например, `Status` из разных доменов).
//! Компилятор предупреждает о неоднозначности, но в этом крейте она намеренна:
//! каждый сервис импортирует только нужный субмодуль.
//!
//! `traits/` — типажи для обобщения по полям: `HasId`, `HasUuid`, `HasStatus` и т.д.
//! Они нужны для generic-кода в репозиториях, который работает с любой сущностью,
//! имеющей, например, числовой `id`.
...
/// Группа секций (модуль системы). Используется для routing и проверки прав —
/// вместо сравнения каждой конкретной `Section` можно сравнивать `SectionKind`.
...
/// Конкретный раздел (экран) системы. Хранится в БД как `i16`.
///
/// `#[repr(i16)]` + `sqlx::Type` — sqlx кодирует/декодирует значение напрямую
/// как числовой тип PostgreSQL без промежуточной строки. `DbEnum` — кастомный
/// derive из `shared-db-derive`, который добавляет методы для работы с БД.
/// `#[db_default]` на `None = 0` — дефолтное значение при null в БД.
#[derive(
    Clone,
    Copy,
    Debug,
