...
use shared_essential::domain::{
    DocumentApproverRep, PlanItemFullRep, PlanRep, PlanRetrospectiveRep,
};
use shared_essential::presentation::dto::processing::{
    InsertUpdateSrmPlansReq, PlanFromSrmParts,
};
...
pub(crate) async fn upsert_legacy_plan(
    request: InsertUpdateSrmPlansReq,
    ...
    super::common::upsert(headers, items, retros, spec_deps, &proc_ctx)
        ...
        .map(|(list, messages)| {
            tracing::info!(
                kind = "insert",
                "Успешно приняты ППЗ с монолита:\n{} заголовков.\n{} позиции",
                list.headers.len(),
                list.items.len()
            );
            messages
...
fn trace_request(req: &InsertUpdateSrmPlansReq) {
    if req.is_empty() {
        tracing::info!(
            kind = "insert",
            "Запрос от монолита на обновление ППЗ: {req:?}\n",
            req = req,
        );
    } else if let Some(item) = req.first() {
        tracing::info!(
            kind = "insert",
            "Запрос от монолита на обновление ППЗ:\n первый заголовок из {req_len}: {header:?}, (Число позиций: {items_len}, Число ретроспектив: {retro_len})",
            req_len = req.len(),
            header = item.header,
...
fn convert_to_local(req: InsertUpdateSrmPlansReq) -> Result<LocalVariables> {
...
fn add_pricing_organization_id(p: &mut PlanRep) {
    use PlanStatus::*;
